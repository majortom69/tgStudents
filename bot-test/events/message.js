// Динамическая загрузка обработчиков евентов с ./message
const fs = require('fs');
...
module.exports = {
    ...
    execute: (bot, msg) => {
        ...
        const handlersDir = path.join(__dirname, 'message');
        const handlerFiles = fs.readdirSync(handlersDir).filter(file => file.endsWith('.js'));

        for (const file of handlerFiles) {
            const handler = require(`./message/${file}`);
            if (handler.step === userState.step) {
                handler.execute(bot, msg, userState);
                return;
            }
        }
